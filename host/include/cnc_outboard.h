/*
 * Open CNC Control — the cyclic channel between the core and the driver
 ...
 * detail. ADR 0009 made the driver process the timebase: it wakes on the
 ...
 *   newest published cycle, and the driver process sends whatever the core has
 ...
 *   all live inside the driver process, which is the only side that knows
 ...
 *   it into the other repository is copying one file. CNC_OUTBOARD_MAX_AXES
 *   below is CNC_MAX_JOINTS from cnc_abi.h, copied for that reason and checked
 *   against the original in this crate's tests.
 *
 *   Nothing else changes license. `cnc-outboard` (the Rust channel) and
 *   `cnc-driver-outboard` (the plugin) stay Apache-2.0, because the process on
 ...
 */
#ifndef CNC_OUTBOARD_H
#define CNC_OUTBOARD_H
 ...
 * the file itself from cnc_fieldbus.h. Nothing about the layout changed: byte
 * for byte, a 2.0.0 region and a 1.1.0 region are the same bytes.
 *
 * It is a major bump anyway, because renaming a symbol is removing one and
 * this file is a contract with programs outside its repository. It was done
 * now because it is the cheapest it will ever be: there was exactly one
 * program on the other side, `driver-hostmot2`, it pins a version, and it is
 * the same author's. Every month of waiting adds another.
 *
 * The reason: the word *fieldbus* described the first thing that was ever on
 * the far side of this channel and stopped being true when the second arrived.
 * A Mesa FPGA card reached by UDP is not a fieldbus, and neither is whatever
 * comes third. What all of them are is a driver mounted *outboard* -- outside
 * the core's process -- and that is the property this file is about.
 */
 ...
 * equal in cnc-outboard-abi's tests, which is where a divergence is caught.
 */
#define CNC_OUTBOARD_MAX_AXES 32

/* Cache line both sides align to. See the padding note on cnc_outboard_shm. */
#define CNC_OUTBOARD_CACHE_LINE 64
 ...
 */
typedef struct cnc_outboard_axis_output {
  double position_cmd;
  double velocity_ff;
  /* cnc_outboard_axis_request. */
  uint32_t request;
  uint32_t reserved;
} cnc_outboard_axis_output;

/*
 * Where a drive is in its state machine, as the core needs to see it. The CiA
 * 402 states themselves stay inside the driver process; these are the
 * distinctions the core acts on.
 */
typedef enum cnc_outboard_axis_state {
  /* Powered down. The axis will not move. */
  CNC_OUTBOARD_AXIS_DISABLED = 0,
  /* Enabled and following setpoints. */
  CNC_OUTBOARD_AXIS_ENABLED = 1,
  /* Faulted. It stopped on its own and needs a reset before it will enable. */
  CNC_OUTBOARD_AXIS_FAULT = 2,
  /* Between states: a transition the drive has not finished. */
  CNC_OUTBOARD_AXIS_TRANSITION = 3
} cnc_outboard_axis_state;

/*
 * What the core is asking of a drive. The driver process owns CiA 402 and
 * turns these into the transitions that reach it; the core says what it wants,
 * not how to get there.
 */
typedef enum cnc_outboard_axis_request {
  CNC_OUTBOARD_REQUEST_DISABLE = 0,
  CNC_OUTBOARD_REQUEST_ENABLE = 1,
  /* Clear a fault. Rising-edge: the drive enables only on a later ENABLE. */
  CNC_OUTBOARD_REQUEST_RESET_FAULT = 2,
  /* Decelerate on the drive's own quick-stop ramp and hold. */
  CNC_OUTBOARD_REQUEST_QUICK_STOP = 3
} cnc_outboard_axis_request;
...
typedef struct cnc_outboard_output {
  cnc_outboard_axis_output axes[CNC_OUTBOARD_MAX_AXES];
} cnc_outboard_output;
 ...
 * same enumerator values, so `cnc-driver-outboard` passes a signal through
 ...
 */
#define CNC_OUTBOARD_SIGNAL_NAME_MAX 64
...
typedef enum cnc_outboard_signal_type {
  CNC_OUTBOARD_SIGNAL_BOOL = 0,
  CNC_OUTBOARD_SIGNAL_I32 = 1,
  CNC_OUTBOARD_SIGNAL_F64 = 2
} cnc_outboard_signal_type;

/* What the signal is for. Values are cnc_signal_role's, from cnc_driver.h. */
typedef enum cnc_outboard_signal_role {
  CNC_OUTBOARD_ROLE_DIGITAL = 0,
  CNC_OUTBOARD_ROLE_ANALOG = 1,
  CNC_OUTBOARD_ROLE_POSITION = 2,
  CNC_OUTBOARD_ROLE_VELOCITY = 3,
  CNC_OUTBOARD_ROLE_TORQUE = 4,
  CNC_OUTBOARD_ROLE_STATUS_WORD = 5,
  CNC_OUTBOARD_ROLE_CONTROL_WORD = 6,
  CNC_OUTBOARD_ROLE_ENCODER_COUNT = 7,
  CNC_OUTBOARD_ROLE_PROBE_LATCH = 8,
  /*
   ...
   * claim this role.
   */
  CNC_OUTBOARD_ROLE_FAST_DIGITAL = 9
} cnc_outboard_signal_role;

/* SI units, per ground rule 3. Values are cnc_unit's, from cnc_abi.h. */
typedef enum cnc_outboard_unit {
  CNC_OUTBOARD_UNIT_NONE = 0,
  CNC_OUTBOARD_UNIT_METRE = 1,
  CNC_OUTBOARD_UNIT_RADIAN = 2,
  CNC_OUTBOARD_UNIT_METRE_PER_SECOND = 3,
  CNC_OUTBOARD_UNIT_RADIAN_PER_SECOND = 4,
  CNC_OUTBOARD_UNIT_NEWTON = 5,
  CNC_OUTBOARD_UNIT_NEWTON_METRE = 6,
  CNC_OUTBOARD_UNIT_VOLT = 7,
  CNC_OUTBOARD_UNIT_AMPERE = 8,
  CNC_OUTBOARD_UNIT_SECOND = 9,
  CNC_OUTBOARD_UNIT_COUNT = 10
} cnc_outboard_unit;

/*
 * One signal. Written once by the driver process before it starts cycling and
 * read-only thereafter, exactly like cnc_outboard_config: the table describes
 ...
 */
typedef struct cnc_outboard_signal_desc {
  /* NUL-terminated. Unique within the region, in either direction. */
  char name[CNC_OUTBOARD_SIGNAL_NAME_MAX];
  /* cnc_outboard_signal_direction. */
  uint32_t direction;
  /* cnc_outboard_signal_type. */
  uint32_t type;
 ...
 *
 * This structure sits immediately after cnc_outboard_shm, at
 * sizeof(cnc_outboard_shm) from the start of the region -- which is a multiple
 * of CNC_OUTBOARD_CACHE_LINE, because that structure's alignment is. It exists
 ...
 */
typedef struct cnc_outboard_signal_block {
  ...
  /* sizeof(cnc_outboard_signal_desc), so a size skew is caught, not suffered. */
  uint32_t desc_size;
...
/*
 * Written once by the driver process at creation and never again. The core
 ...
 */
typedef struct cnc_outboard_config {
  uint32_t version_major;
  uint32_t version_minor;
  uint32_t version_patch;
  /* Axes actually in use. The arrays above are always CNC_OUTBOARD_MAX_AXES. */
  uint32_t axis_count;
  /*
   * Identifies this run of the driver process.
   *
   ...
   * it did not attach to: an outboard restart is a fault that reaches the
  ...
  uint64_t generation;
  /*
   * sizeof(struct cnc_outboard_shm), checked at attach against the mapping.
   *
   * The fixed part only. From 1.1.0 the object is larger than this whenever
   * the process declares signals; `cnc_outboard_signal_block.region_size` says
  ...
  /*
   * Unanswered cycles before the driver process takes the drives down
   ...
   * and `cnc-driver-outboard` reports it as its own, so the budget check
 ...
 *
 * The driver process writes input[n & 1] and then stores cycle = n with release
 ...
 * ordering. At its send deadline the driver process loads response with acquire
 * ordering and reads output[response & 1] — this cycle's work if the core
 * finished, and the previous cycle's if it did not.
 *
 * The driver process may publish cycle n only while response >= n - 1. That guard is
 ...
 *
 *     0                          cnc_outboard_shm          (this structure)
 *     sizeof(cnc_outboard_shm)   cnc_outboard_signal_block (where the rest is)
 *     block.table_offset         cnc_outboard_signal_desc[block.count]
 ...
 */
typedef struct cnc_outboard_shm {
  cnc_outboard_config config;

  /* Outboard -> core. Accessed atomically; see the ordering note above. */
  alignas(CNC_OUTBOARD_CACHE_LINE) uint64_t cycle;
  alignas(CNC_OUTBOARD_CACHE_LINE) cnc_outboard_input input[2];

  /* Core -> outboard. */
  alignas(CNC_OUTBOARD_CACHE_LINE) uint64_t response;
  alignas(CNC_OUTBOARD_CACHE_LINE) cnc_outboard_output output[2];
  ...
  alignas(CNC_OUTBOARD_CACHE_LINE) uint32_t state;
} cnc_outboard_shm;
 ...
 */
typedef enum cnc_outboard_channel_state {
  /* The region exists but the driver process has not started cycling. */
  CNC_OUTBOARD_STATE_STARTING = 0,
  /* Cycling. */
  CNC_OUTBOARD_STATE_RUNNING = 1,
  /* One side is going away by intent. */
  CNC_OUTBOARD_STATE_SHUTDOWN = 2,
  /* One side gave up. Not resumable: the region is recreated on restart. */
  CNC_OUTBOARD_STATE_FAULTED = 3
} cnc_outboard_channel_state;
